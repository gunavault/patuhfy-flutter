...
part 'real_pemeliharaan_jalan_form_state.dart';

class RealPemeliharaanJalanFormCubit extends Cubit<RealPemeliharaanJalanFormState> {
  final LocalDataSource localDataSource;
  final RemoteDataSource remoteDataSource;

  RealPemeliharaanJalanFormCubit(this.localDataSource, this.remoteDataSource)
      : super(InitialRealPemeliharaanJalanFormState());

  storedOffline(RealPemeliharaanJalanFormModel dataForm) async {
    ...
    List<RealPemeliharaanJalanFormModel> cekLength;
    cekLength = await localDataSource
        .getDataRealPemeliharaanJalanByTanggal(dataForm.tanggal.toString());

    if (cekLength.isEmpty) {
      //Check duplikat
      await localDataSource.addDataRealPemeliharaanJalan(dataForm);
    }

    emit(SuccessRealPemeliharaanJalanFormState(
        status_code: 200, message: 'Inserted to Lokal Database'));

    print('bepraisi data ${cekLength.length}');
  }

  storedOnline(RealPemeliharaanJalanFormModel dataForm, UserModel userModel) async {
    ...
    RealPemeliharaanJalanFormModelResponse resFromApi =
    ...
    if (resFromApi.status_code == 200) {
      ...
      emit(DuplicatedRealPemeliharaanJalanFormState(
          status_code: resFromApi.status_code, message: resFromApi.message));
    }
  }

  submitToDatabase(RealPemeliharaanJalanFormModel dataForm) async {
    try {
      emit(LoadingRealPemeliharaanJalanFormState());
      // SharedPreferences prefs = await SharedPreferences.getInstance();
      ...
      if (connectivityResult != ConnectivityResult.none) {
        localDataSource.deleteDataRealPemeliharaanJalanByDate(
            dataForm.tanggal.toString()); //Hapus data di Lokal By Date
        emit(ErrorRealPemeliharaanJalanFormState(err.toString())); //Emit Error State
      } else {
        emit(SuccessRealPemeliharaanJalanFormState(
            status_code: 200, message: 'Inserted to Lokal Database'));
      }
  ...
  // createRealPemeliharaanJalan(
  //     RealPemeliharaanJalanFormModel dataFormRealPemeliharaanJalan, String kode_sales_order) async {
