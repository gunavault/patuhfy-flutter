...
class FormPenyelesaiankerusakanAlatForm extends StatelessWidget {
  FormPenyelesaiankerusakanAlatForm({Key? key, required this.dataPelaporanKerusakanAlat}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    TextEditingController keteranganpenyelesaianController= TextEditingController();
    ...
    void _postToDatabase() {
      ...
      context.read<PenyelesaiankerusakanAlatFormCubit>().submitToDatabase(
            PenyelesaianKerusakanAlatModel(
              rowslaporan: rowstampAcuanController.text,
              foto: imageNameController.text,
              keteranganpenyelesaian: keteranganpenyelesaianController.text,
            ),
          );
    }
    ...
    void _submit() {
      ...
      if (form!.validate()) {
        ...
        void onChangeSelectKeteranganPenyelesaian(value) {
      keteranganpenyelesaianController.text = value!.toString();
    }
    ...
    void _onSuccessSubmit(message) {
      showAlertSuccessOkActionV2(context, message, () {
        BlocProvider.of<PenyelesaiankerusakanAlatListCubit>(context).getData(dataPelaporanKerusakanAlat);
    ...
    return GestureDetector(
      ...
      child: BlocListener<PenyelesaiankerusakanAlatFormCubit, PenyelesaiankerusakanAlatFormState>(
        listener: (context, PenyelesaiankerusakanAlatFormState) {
          if (PenyelesaiankerusakanAlatFormState is LoadingPenyelesaiankerusakanAlatFormState) {
            print('ke sini');
          ...
          } else if (PenyelesaiankerusakanAlatFormState is SuccessPenyelesaiankerusakanAlatFormState) {
            ScaffoldMessenger.of(context).hideCurrentSnackBar();
            _onSuccessSubmit(PenyelesaiankerusakanAlatFormState.message);
            // BlocProvider.of<PelaporanKerusakanAlatListCubit>(context).getData();
            BlocProvider.of<AnalisakerusakanAlatListCubit>(context).getData(dataPelaporanKerusakanAlat);
            
            // showAlertSuccessOkActionV2(context, PenyelesaiankerusakanAlatFormState.message, () {
            //   Navigator.pop(context);
            // });
          } else if (PenyelesaiankerusakanAlatFormState is DuplicatedPenyelesaiankerusakanAlatFormState) {
          ...
          } else if (PenyelesaiankerusakanAlatFormState is ErrorPenyelesaiankerusakanAlatFormState) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(PenyelesaiankerusakanAlatFormState.message.toString()),
        ...
        child: LoaderOverlay(
          ...
          child: Scaffold(
            ...
            appBar: AppBarView(
              title: "Form Penyelesaian Kerusakan Alat",
                ...
                child: Form(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      children: [
                        SelectboxSukuCadang(
                          titleName: "Keterangan Penyelesaian",
                          isTitleName: true,
                          onChangeFunc: onChangeSelectKeteranganPenyelesaian),
                        UploadFoto(
